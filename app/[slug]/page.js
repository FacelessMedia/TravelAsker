...
export default async function PostPage({ params }) {
  ...
  // Insert TOC after first paragraph — only for blog posts (not static pages) with 3+ headings
  const isPost = !!post;
  const showTOC = isPost && headings.length >= 3;
        ...
        <div className="post-container">
          ...
          <article className="post-article" itemScope itemType="https://schema.org/Article">
              ...
              {(item.authorDisplay || item.date) && (
                <div className="post-meta">
                  ...
                  <div className="post-share">
                    ...
                    <a href={`mailto:?subject=${encodeURIComponent('Your friend has shared an article with you.')}&body=${encodeURIComponent(item.title)}%0D%0A${encodeURIComponent(`${SITE_URL}/${item.slug}/`)}%0D%0A`} className="share-btn share-email" aria-label="Share via Email">
            ...
            <div className="post-content" itemProp="articleBody">
              {showTOC && finalContent.includes('{{TOC_PLACEHOLDER}}') ? (
                <>
            ...
            {isPost && item.authorDisplay && (
              <div className="author-bio">
                <div className="author-bio-info">
                  <h3 className="author-bio-name">
                    <Link href={`/author/${item.author.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}/`}>{item.authorDisplay}</Link>
                  </h3>
                </div>
              </div>
            )}
